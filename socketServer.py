...
class ThreadedTCPRequestHandler(SocketServer.BaseRequestHandler):
	...
	def ospfProcessing(self, query):
		...
		srcMask, dstMask = query.get("srcMask"), query.get("dstMask")
		srcAs, dstAs = query.get("srcAs"), query.get("dstAs")
		srcRouterId, dstRouterId = query.get("srcRouterId"), query.get("dstRouterId")
		if isHQ==1:
			print "Is HQ ^_^"
			...
			r = RouteAnalyser()
			result = r.getOverallRoute(pid, srcIp, dstIp, srcMask, dstMask, srcAs, dstAs, srcRouterId, dstRouterId)
			result = r.formatResult(result)
			print result
		else:
			print "Not HQ >_<"
			t = OspfTopoGenerator()
			t.connectDB(sc.getDBConfig())
			t.makeOspfTopo(pid)
			result = t.getShortestPaths(srcRouterId, dstRouterId)
		self.request.sendall(json.dumps(result))
	...
	def isisProcessing(self, query):
		...
		t.connectDB(sc.getDBConfig())
		t.makeIsIsTopo(pid)
		result = t.getShortestPaths(srcIp, dstIp, srcRouterId, dstRouterId)
		self.request.sendall(json.dumps(result))
